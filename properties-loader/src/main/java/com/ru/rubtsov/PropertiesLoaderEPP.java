...
public class PropertiesLoaderEPP implements EnvironmentPostProcessor {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        Arrays.stream(getResources()).map(this::loadPropertySources).forEach(environment.getPropertySources()::addLast);
    }
    ...
    private Resource[] getResources(){
        try {
           return new PathMatchingResourcePatternResolver().getResources(LOCATION_PATTERN);
            ...
            throw new IllegalStateException("Failed to load properties configuration from " + PATH, exception);
        }
    }
